...
class TestAdaptiveRegression(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        model = AdaptiveRegression(epochs=20, learning_rate=0.01)
    ...
    def test_with_regularization(self):
        ...
        model = AdaptiveRegression(
            epochs=20, learning_rate=0.01, data_regularization=True
        )
        model.fit(self.X, self.y)
        predictions = model.predict(self.X)
    ...
    def test_without_regularization(self):
        ...
        model = AdaptiveRegression(
            epochs=20, learning_rate=0.01, data_regularization=False
        )
        model.fit(self.X, self.y)
        predictions = model.predict(self.X)
