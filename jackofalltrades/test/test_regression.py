...
from jackofalltrades.Models import (
    LinearRegression, LogisticRegression, MLPRegressor, 
    RidgeRegression, AdaptiveRegression
)
...
class TestLogisticRegression(unittest.TestCase):
    ...
    def test_fit_predict(self):
        """Test model fitting and prediction."""
        model = LogisticRegression(epochs=100, learning_rate=0.1)  # Reduced epochs
        model.fit(self.X, self.y)
        predictions = model.predict(self.X)
    ...
    def test_predict_binary(self):
        ...
        model = LogisticRegression(epochs=100, learning_rate=0.1)
        model.fit(self.X, self.y)
